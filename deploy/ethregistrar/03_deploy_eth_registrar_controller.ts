...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const resolver = await registry.resolver(ethers.utils.namehash('vcl'))
  if (resolver === ethers.constants.AddressZero) {
    console.log(
      `No resolver set for .vcl; not setting interface ${interfaceId} for ETH Registrar Controller`,
    )
    return
  }
  const resolverContract = await ethers.getContractAt('OwnedResolver', resolver)
  const tx3 = await resolverContract.setInterface(
    ethers.utils.namehash('vcl'),
    interfaceId,
    controller.address,
  )
  console.log(
    `Setting ETHRegistrarController interface ID ${interfaceId} on .vcl resolver (tx: ${tx3.hash})...`,
  )
  await tx3.wait()
}
